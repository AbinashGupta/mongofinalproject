...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        database.collection('item').find(catQuery)
        .sort({_id: 1})
    ...
    this.getNumItems = function(category, callback) {
        ...
        database.collection('item').count(catQuery, (err, numItems) => {
            if (!err) {
                callback(numItems);
            }
        });
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        ...
        database.collection('item').count({$text:{$search:query}}, (err, numItems) => {
            if (!err) {
                callback(numItems);
            }
        });
    ...
    this.getItem = function(itemId, callback) {
         ...
         *
         */
        database.collection('item').findOne({_id: itemId}, (err, item) => {
            if (!err) {
                callback(item);
            }
        })
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        database.collection("item").updateOne({_id: itemId}, {$push: {reviews: reviewDoc}}, (err, doc) => { if (!err) callback(doc);})
